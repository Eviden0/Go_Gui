...
func (a *App) GetBdOCRToken() (bool, string) {
	...
	if err != nil {
		a.Log.Errorf(configs.BdOcrTokenGetErr, err.Error())
		return false, err.Error()
	}
...
func (a *App) BaiduOCR(ocrType int8) *util.Resp {
	...
	uri, err := url.Parse(host)
	if err != nil {
		a.Log.Errorf(configs.BdOcrUrlParseErr, err.Error())
	...
	if err != nil {
		...
		a.Log.Errorf(configs.BdOcrRequestErr, err.Error())
		return util.Error(err.Error())
	}
	defer response.Body.Close()
	result, err := io.ReadAll(response.Body)
	if err != nil {
		...
		a.Log.Errorf(configs.BdOcrResponseErr, err.Error())
		return util.Error(err.Error())
	}
	var msg string = string(result[:])
	if strings.Contains(msg, "error_msg") {
		a.ConfigMap["bdocr"]["token"] = ""
		a.UpdateConfigItem(internal.ConfigItem {
			Name: "token",
			Type: "bdocr",
		...
		a.Log.Errorf(configs.BdOcrResponseErr, msg)
		return util.Error("OCR识别失败, 请重试")
	}
