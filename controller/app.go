...
func (a *App) initTrans() {
	switch a.ConfigMap["trans"]["transType"] {
	case "bdtrans":
		var trans ITrans = &BdTransClient{
			ConfigMap: a.ConfigMap["bdtrans"],
			Log:       a.Log,
		}
		a.Trans = &trans
	case "txtrans":
		var trans ITrans = &TxTransClient{
			ConfigMap: a.ConfigMap["txtrans"],
			Log:       a.Log,
		}
		a.Trans = &trans
	case "alitrans":
		var trans ITrans = &AliTransClient{
			ConfigMap: a.ConfigMap["alitrans"],
			Log:       a.Log,
		}
		a.Trans = &trans
	default:
		a.Log.Info(configs.NoTransConfigErr)
	}
}
...
func (a *App) initClipboard() {
	...
	err := clipboard.Init()
	if err != nil {
		a.Log.Errorf(configs.InitSysClipboardErr, err.Error())
		panic(err)
	}
}
