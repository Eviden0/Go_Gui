...
func (a *App) GetConfigByType() *util.Resp {
...
func (a *App) UpdateConfigByType(param updatParam) *util.Resp {
	...
	if strings.Contains(param.Type, "trans") {
		a.initTrans()
	}
	return util.Success(a.ConfigMap)
}
...
func (a *App) initConfigData(ctype string) {
	...
	case "txtrans": // 腾讯翻译配置项
		datas := make([]*internal.ConfigItem, 0)
		for i := 0; i < len(configs.TxTrans); i++ {
			var value string
			switch configs.TxTrans[i] {
			case "region":
				value = "ap-beijing"
			case "from":
				value = "en"
			...
			datas = append(datas, &internal.ConfigItem{Name: configs.TxTrans[i], Value: value, Type: ctype})
		}
		if _, err := a.Db.Insert(&datas); err != nil {
			a.Log.Error(fmt.Sprintf(configs.AddConfigItemErr, ctype, err.Error()))
		}
	case "alitrans": // 阿里翻译配置项
		datas := make([]*internal.ConfigItem, 0)
		for i := 0; i < len(configs.AliTrans); i++ {
			var value string
			switch configs.AliTrans[i] {
			case "region":
				value = "cn-beijing"
			case "type":
				value = "text"
			...
			datas = append(datas, &internal.ConfigItem{Name: configs.AliTrans[i], Value: value, Type: ctype})
		}
		if _, err := a.Db.Insert(&datas); err != nil {
			a.Log.Error(fmt.Sprintf(configs.AddConfigItemErr, ctype, err.Error()))
		}
	...
	case "mdconf": // markdown编辑器的配置项
		datas := make([]*internal.ConfigItem, 0)
		for i := 0; i < len(configs.MdConf); i++ {
			var value string
			switch configs.MdConf[i] {
			case "mdconf":
				value = "light"
			default:
				value = ""
			}
			datas = append(datas, &internal.ConfigItem{Name: configs.MdConf[i], Value: value, Type: ctype})
		}
		if _, err := a.Db.Insert(&datas); err != nil {
			a.Log.Error(fmt.Sprintf(configs.AddConfigItemErr, ctype, err.Error()))
		}
		case "sysconf": // markdown编辑器的配置项
		datas := make([]*internal.ConfigItem, 0)
		for i := 0; i < len(configs.SysConf); i++ {
			var value string
			switch configs.MdConf[i] {
			case "mdconf":
				value = "light"
			...
			datas = append(datas, &internal.ConfigItem{Name: configs.SysConf[i], Value: value, Type: ctype})
		}
		if _, err := a.Db.Insert(&datas); err != nil {
			a.Log.Error(fmt.Sprintf(configs.AddConfigItemErr, ctype, err.Error()))
		}
